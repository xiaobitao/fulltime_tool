...
internal_alarms =  metadata.tables['internal_alarms']
...
def list_internal_alarms(channel, sensor, date_range = None, **kwargs):
    """ Internal alarm Events
    """

    # TODO(Gang): implement the filter logic.

    stmt = internal_alarms.select().order_by(sa.asc(internal_alarms.c.sDatetime))
    rows = engine.execute(stmt).fetchall()
    print("event result size:", len(rows))
    if len(rows) > 0:
